<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Monitoring Order</h2>
    </x-slot>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            @forelse ($orders as $order)
                                <tr>
                                    ...
                                    <td class="px-4 py-4">
                                        ...
                                        <p class="text-xs text-gray-500">{{ $order->seller->sellerProfile->shop_name ?? '-' }}</p>
                                    </td>
                                    <td class="px-4 py-4 text-gray-600">{{ $order->expeditionLabel() }}</td>
                                    <td class="px-4 py-4">
                                        <form action="{{ route('admin.orders.shipping.update', $order) }}" method="POST" class="flex items-center gap-2">
                                            @csrf
                                            @method('PATCH')
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-4 py-10 text-center text-gray-500">Belum ada order.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
