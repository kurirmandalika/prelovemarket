...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="min-h-screen overflow-x-hidden bg-zinc-50 font-sans text-zinc-950 antialiased">
        <div class="flex min-h-screen flex-col pb-16 lg:pb-0">
            <header x-data="{ mobileMenuOpen: false }" class="sticky top-0 z-40 border-b border-zinc-200/80 bg-white/95 backdrop-blur-xl">
                <div class="mx-auto max-w-7xl px-4 py-3 sm:px-6 lg:px-8">
                    <div class="flex items-center justify-between gap-3">
                        <a href="{{ route('home') }}" class="group inline-flex items-center gap-3">
                            <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-md bg-zinc-950 text-sm font-extrabold text-white shadow-sm shadow-emerald-900/20 transition group-hover:bg-emerald-700 sm:h-11 sm:w-11">PM</span>
                            <span class="leading-tight">
                                <span class="block text-sm font-extrabold text-zinc-950 sm:text-base">Preloved Market</span>
                                <span class="block max-w-[10rem] truncate text-xs font-medium text-zinc-500 sm:max-w-none">Curated second-life goods</span>
                            </span>
                        </a>
                        ...
                        <button
                            ...
                            class="inline-flex h-10 items-center justify-center rounded-md border border-zinc-200 px-3 text-sm font-extrabold text-zinc-700 shadow-sm transition hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-800 lg:hidden"
                    ...
                    <nav
                        ...
                        class="mt-3 grid gap-2 rounded-md border border-zinc-200 bg-white p-2 text-sm font-extrabold shadow-lg shadow-zinc-200 lg:hidden"
                    >
                        <a href="{{ route('home') }}" class="rounded-md px-3 py-3 transition {{ request()->routeIs('home') ? 'bg-zinc-950 text-white shadow-sm' : 'text-zinc-700 hover:bg-zinc-100' }}">Home</a>
                        <a href="{{ route('products.index') }}" class="rounded-md px-3 py-3 transition {{ request()->routeIs('products.*') || request()->routeIs('categories.*') ? 'bg-zinc-950 text-white shadow-sm' : 'text-zinc-700 hover:bg-zinc-100' }}">Produk</a>

                        @auth
                            <a href="{{ route('dashboard') }}" class="rounded-md px-3 py-3 text-zinc-700 transition hover:bg-zinc-100">Dashboard</a>
                            @if (auth()->user()->isAdmin())
                                <a href="{{ route('admin.dashboard') }}" class="rounded-md bg-amber-400 px-3 py-3 text-zinc-950 shadow-sm transition hover:bg-amber-300">Admin</a>
                            @endif
                        @else
                            <a href="{{ route('login') }}" class="rounded-md px-3 py-3 text-zinc-700 transition hover:bg-zinc-100">Login</a>
                            <a href="{{ route('register') }}" class="rounded-md bg-emerald-700 px-3 py-3 text-center text-white shadow-sm shadow-emerald-900/20 transition hover:bg-emerald-800">Mulai Jual</a>
                        @endauth
                    </nav>
                </div>
            </header>
            ...
            <nav class="fixed inset-x-0 bottom-0 z-50 border-t border-zinc-200 bg-white/95 px-3 py-2 shadow-2xl shadow-zinc-950/10 backdrop-blur lg:hidden">
                <div class="mx-auto grid max-w-md grid-cols-3 gap-2 text-center text-xs font-extrabold">
                    <a href="{{ route('home') }}" class="rounded-md px-2 py-2 transition {{ request()->routeIs('home') ? 'bg-zinc-950 text-white' : 'text-zinc-600 hover:bg-zinc-100' }}">Home</a>
                    <a href="{{ route('products.index') }}" class="rounded-md px-2 py-2 transition {{ request()->routeIs('products.*') || request()->routeIs('categories.*') ? 'bg-zinc-950 text-white' : 'text-zinc-600 hover:bg-zinc-100' }}">Produk</a>
                    @auth
                        <a href="{{ route('dashboard') }}" class="rounded-md px-2 py-2 transition {{ request()->routeIs('dashboard*') ? 'bg-emerald-700 text-white' : 'text-zinc-600 hover:bg-zinc-100' }}">Dashboard</a>
                    @else
                        <a href="{{ route('register') }}" class="rounded-md bg-emerald-700 px-2 py-2 text-white transition hover:bg-emerald-800">Jual</a>
                    @endauth
                </div>
            </nav>
        </div>
